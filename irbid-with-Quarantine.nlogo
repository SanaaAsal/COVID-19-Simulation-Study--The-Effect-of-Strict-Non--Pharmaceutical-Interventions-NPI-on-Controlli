...
to do-6am-Acts
; if ((occupation = 500)  and ( not (disease-status = 1)) and ( not (disease-status = 2) ));boys go to help out in the kitchen. All others should be at home
...
;if ( ( not (disease-status = 1)) and ( not (disease-status = 2) ) )
;[move-home]
end
...
[if (( ticks > 72 ) and ( not (disease-status = 1) and ( not (disease-status = 2))))
...
;  [move-home ]
 ...
 [set boy-cutoff (age * 0.08) - 0.22
  set boy-prob random-float 1.0
   if ((boy-prob < boy-cutoff) and ( not (disease-status = 1)) and ( not (disease-status = 2) ))
    [move-to-bldg (fields-id)]
   ; [move-to-bldg (kitchen-id)]
  ]
...
to do-MSat11am4pm-Acts ;nearly the same as do-MSat910am23pm-Acts, except boys must move to the kitchen at this time to help with meal prep
 if ((occupation = 100) or (occupation = 300) and not (disease-status = 2))
  [move-to-bldg (fields-id)]
...
to do-MSat1pm-Acts ; All school-aged children and the priest go to the church building for catechism, everyone else moves within their dwelling
 if ((occupation = 500) or (occupation = 600) or (occupation = 800) and ( not (disease-status = 1)) and ( not (disease-status = 2) ) )
  ; [move-to-bldg (church-id)]
  []
 ; [move-home]
end
...
; if ((occupation <= 300) and ( not (disease-status = 1)) and ( not (disease-status = 2)  ))
 ; [if (can-visit? and (random-float 1.0 <= prob-visit))
   ; [ move-visit ]
  ;  [move-to-bldg (present-location)
  ;  ]
...
; if ((occupation = 400) or (occupation = 800)and ( not (disease-status = 1)) and ( not (disease-status = 2)  ))
;   [move-home]
end
...
;  [ ; opens else of disease-status = 1
  ...
  ;ask (patch-at 0 0) [ set pcolor gray - 3 ]
    let death-threshold random-float 1.0
      ifelse (age >= 65) [set adjusted-death-prob death-prob * 1.25] ;gives adults a higher prob of death as it is written. Can also be used to give older folks less chance of dying
